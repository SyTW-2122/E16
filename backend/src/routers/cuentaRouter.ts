...
cuentaRouter.get('/cuenta', async (req, res) => {
  ...
  try {
    ...
    if (cuentasMatch.length !== 0) { return res.json(cuentasMatch); }
...
cuentaRouter.patch('/cuenta', async (req, res) => {
  ...
  let newNickname = "";
  if (typeof req.body.username != undefined) { newNickname = req.body.username; } 
  else { newNickname = cuentaEncontrada.username; }

  let newName = "";
  if (typeof req.body.name != undefined) { newName = req.body.name; }
  else { newName = cuentaEncontrada.name; }

  let newSurname = "";
  if (typeof req.body.surname != undefined) { newSurname = req.body.surname;} 
  else { newSurname = cuentaEncontrada.surname; }

  let newAddress = "";
  if (typeof req.body.address != undefined) { newAddress = req.body.address; } 
  else { newAddress = cuentaEncontrada.address; }
